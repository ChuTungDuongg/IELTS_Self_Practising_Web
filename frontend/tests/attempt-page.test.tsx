...
describe("attempt page initial state", () => {
  ...
  it.each(["SUBMITTED", "AUTO_SUBMITTED", "INTERRUPTED", "ABANDONED"])("redirects terminal %s standalone attempts before rendering a runner", async (status) => {
    vi.mocked(getExam).mockResolvedValue({ attempt: { attempt_id: attemptId, status, test_session_id: null } } as never);
    await expect(AttemptShellPage(params)).rejects.toThrow(`redirect:/review/${attemptId}`);
  });
  ...
  it("returns a terminal Full Mock attempt to session advancement", async () => {
    ...
    await expect(AttemptShellPage(params)).rejects.toThrow(`redirect:/test-session/${sessionId}`);
  });
