...
export function ListeningRunner({ initial }: { initial: ExamPayload }) {
  ...
  return <div className="exam-runner listening-exam"><header className="exam-header"><div><p>LISTENING · SECTION {part.order_index + 1}</p><h1>{initial.test_title}</h1></div><div className="exam-header-tools"><ThemeToggle /><div className="exam-header-status"><span className={`exam-timer ${initial.attempt.timer_mode === "COUNTDOWN" && seconds < 300 ? "exam-timer-warning" : ""}`}>{formatDuration(seconds)}</span><span className={`exam-save-state exam-save-${saveState}`}>{saveState === "saving" ? "Saving…" : saveState === "error" ? "Save failed" : "Saved"}</span></div></div></header>
    {initial.listening_audio_asset ? <ListeningAudioPlayer src={assetContentUrl(initial.listening_audio_asset)} /> : <p className="notice m-4">This Listening test has no audio recording attached.</p>}
