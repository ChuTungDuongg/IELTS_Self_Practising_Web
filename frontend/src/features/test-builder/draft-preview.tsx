...
export function DraftPreview({ version, moduleType }: { version: BuilderVersion; moduleType: "READING" | "LISTENING" }) {
  ...
  return <div className="exam-runner draft-preview">
    ...
    <footer className="exam-footer"><div>{sections.map((item, index) => <button key={item.id} onClick={() => setSectionIndex(index)} className={index === sectionIndex ? "active" : ""}>{moduleType === "READING" ? "Passage" : "Section"} {item.order_index + 1}</button>)}</div><span>Answers in preview are not saved.</span></footer>
  </div>;
}
