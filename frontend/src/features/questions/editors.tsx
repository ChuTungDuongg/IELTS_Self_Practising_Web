...
function SingleOptionAnswerSelect({ question, options, onChange }: { question: QuestionModel; options: Option[]; onChange: (value: string) => void }) {
  const selected = singleOptionValue(question);
  const unavailable = selected && !options.some((option) => option.id === selected);
  return <select aria-label={`Question ${question.number} correct option`} className="select-field" value={selected} onChange={(event) => onChange(event.target.value)}>
    <option value="">Choose option</option>
    {unavailable ? <option value={selected} disabled>Unavailable option</option> : null}
    {options.map((option) => <option key={option.id} value={option.id}>{option.label} — {option.text}</option>)}
  </select>;
}
