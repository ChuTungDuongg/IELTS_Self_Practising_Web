...
export function ReadingReviewView({ data }: { data: ReviewData }) {
  ...
  return <div className="space-y-8">
    ...
    <div className="listening-part-tabs">{data.passages.map((item, index) => <button key={item.id} className={index === passageIndex ? "active" : ""} onClick={() => setPassageIndex(index)}><b>Passage {index + 1}</b><span>{item.question_groups.flatMap((group) => group.questions).length} questions</span></button>)}</div>
