...
from app.schemas.tests import TestCreate, TestSummary, VersionCreate, VersionDetail
...
async def list_tests(session: AsyncSession = Depends(get_session)) -> list[TestSummary]:
    records = await TestService(session).list_tests()
