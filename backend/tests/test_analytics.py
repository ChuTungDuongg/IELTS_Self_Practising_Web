...
async def test_analytics_excludes_active_and_null_bands_and_applies_weak_threshold(
    ...
    group = QuestionGroup(question_type="matching_headings", instruction="", config={}, order_index=0)
    group.questions.extend(
        Question(number=index + 1, prompt=f"Q{index + 1}", config={}, answer_key={"kind": "SINGLE_OPTION", "value": "x"}, order_index=index)
        for index in range(5)
    )
    module.question_groups.append(group)
    version.modules.append(module)
    test.versions.append(version)
    finalized = Attempt(test_version=version, module_type=ModuleType.READING, timer_mode=TimerMode.COUNT_UP, started_at=now, last_active_at=now, finished_at=now + timedelta(seconds=100), elapsed_seconds=100, status=AttemptStatus.SUBMITTED, finished_reason=FinishedReason.USER_SUBMIT, band_score=Decimal("7.0"))
    active = Attempt(test_version=version, module_type=ModuleType.READING, timer_mode=TimerMode.COUNT_UP, started_at=now, last_active_at=now, status=AttemptStatus.IN_PROGRESS, band_score=Decimal("9.0"))
    finalized.answers.extend(AttemptAnswer(question=question, value="x", is_correct=index < 3) for index, question in enumerate(group.questions))
    ...
    matching = next(item for item in dashboard.question_types if item.question_type == "matching_headings")
    baseline_matching = next((item for item in baseline.question_types if item.question_type == "matching_headings"), None)
