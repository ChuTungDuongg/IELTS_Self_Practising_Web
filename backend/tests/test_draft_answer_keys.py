    ...
    source = DraftImportManifest.model_validate({
        ...
        "modules": [{
            ...
            "passages": [{
                ...
                "blocks": [{"type": "paragraph", "label": "A", "text": "Fictional content."}],
...
async def test_empty_and_incremental_single_option_keys_remain_draft_saveable(db_session):
    version_id, group = await import_group(db_session, {
        "question_type": "multiple_choice",
        "questions": [
            {"prompt": "Fictional question one", "options": [{"key": "A", "text": "River"}, {"key": "B", "text": "Hill"}]},
            {"prompt": "Fictional question two", "options": [{"key": "A", "text": "River"}, {"key": "B", "text": "Hill"}]},
        ],
    })
    ids = [question.id for question in group.questions]
    assert all(question.answer_key == {"kind": "SINGLE_OPTION", "value": ""} for question in group.questions)
    ...
    group = await save_group(db_session, group, keys=[
    ...
    assert any(issue.path == "reading.questions.2" and issue.message == "Answer key is incomplete." for issue in validation.errors)
...
async def test_empty_text_key_survives_draft_update(db_session):
    ...
    assert group.questions[0].answer_key == {"kind": "TEXT", "accepted": [], "case_sensitive": False}
...
async def test_grouped_multi_select_keys_save_at_zero_one_and_two_official_choices(db_session):
    ...
    assert group.questions[0].answer_key == {"kind": "MULTIPLE_OPTIONS", "values": [], "order_matters": False}
    zero = await LifecycleService(db_session).validate(version_id)
    assert any(issue.message == "Select exactly 2 official answers before publishing." for issue in zero.errors)
    group = await save_group(db_session, group, keys=[{"kind": "MULTIPLE_OPTIONS", "values": [values[0]], "order_matters": False}])
    assert group.questions[0].answer_key["values"] == [values[0]]
    one = await LifecycleService(db_session).validate(version_id)
    assert any(issue.message == "Select exactly 2 official answers before publishing." for issue in one.errors)
    group = await save_group(db_session, group, keys=[{"kind": "MULTIPLE_OPTIONS", "values": values[:2], "order_matters": False}])
...
async def test_content_only_draft_edit_preserves_imported_reading_number_gap(db_session):
    source = DraftImportManifest.model_validate({
        ...
        "modules": [{
            ...
            "passages": [{
                ...
                "blocks": [{"type": "paragraph", "label": "A", "text": "Fictional content."}],
                "question_groups": [
                    {"question_type": "short_answer", "questions": [{"number": 1, "prompt": "First place"}]},
                    {"question_type": "short_answer", "questions": [{"number": 14, "prompt": "Later place"}]},
                ],
            }],
        }],
    })
    result = await DraftImportService(db_session).import_manifest(source, Path.cwd())
    groups = (await ReadingService(db_session).builder_version(result.version_id)).modules[0].passages[0].question_groups
    ...
    refreshed = (await ReadingService(db_session).builder_version(result.version_id)).modules[0].passages[0].question_groups
    assert saved.questions[0].number == 14
    assert [(group.questions[0].id, group.questions[0].number) for group in refreshed] == [
        (first_id, 1), (later_id, 14),
    ]
    assert saved.questions[0].answer_key == {"kind": "TEXT", "accepted": [], "case_sensitive": False}
